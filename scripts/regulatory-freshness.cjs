 ...
 * guardian. Reads kb/regulatory-ledger.json and FAILS (exit 1) when any
...
const ledger = JSON.parse(fs.readFileSync(path.join(ROOT, "kb/regulatory-ledger.json"), "utf8"));
  ...
  if (overdueBy > 0) {
    console.log(
      `FAIL  ${entry.id}: OVERDUE ${overdueBy}d (last verified ${entry.lastVerified}, interval ${entry.reviewIntervalDays}d)\n` +
        ...
        `      → then update lastVerified in kb/regulatory-ledger.json`,
    );
    failures += 1;
