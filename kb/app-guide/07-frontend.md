...
|-----------|------:|--------------------|
| `public/` | 13 | Logged-out: landing, persona landing pages (`/refinance` — calculator + dual CTAs; `/va-loans` — military pre-screen prefills `/apply?type=va`; `/self-employed` — income-shape pre-screen prefills `/apply?type=self-employed`; all no global nav), affordability check ("Can I Afford This Home?"), find-an-agent, auth pages |
