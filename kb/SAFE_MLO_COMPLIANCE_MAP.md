...
| **TRID / Know Before You Owe** — LE/CD content + timing | 🟡 LE generation exists behind the eDisclosure (ESIGN) consent gate (L2); CD field numbering referenced in UCD work. **Not built:** the 3-business-day LE timing clock, changed-circumstance re-disclosure tracking, CD 3-day waiting period. These become hard requirements the day real applications flow — see ticket. | `server/routes/lending.ts` (LE), `docs/fannie-mae/` UCD docs |
...
| **Reg N — Mortgage Acts & Practices (advertising)** | 🟡 Trigger-term + consent-language discipline on all four persona LPs (verified: calculators use borrower-entered hypothetical rates only). ⚠️ Found 2026-07-04: the site footer says Homiquity products "are direct loans" — wrong for a broker; ticketed with the NMLS-ID display work (#33). Ad retention policy: 👤 ops. | LP pages; `client/src/components/Footer.tsx:69-71` |
...
- **NMLS unique identifier display** (website, advertisements, loan documents): 🎫 ticketed
  as roadmap #33 — plumbing built now, lights up when F1 assigns real IDs. Company ID lives
  at `server/config/company.ts` (`nmlsId: "PENDING"`); LO NMLS IDs exist on staff profiles
  (`AdminUsers.tsx`, `AgentCoBranding.tsx` render them where present).
