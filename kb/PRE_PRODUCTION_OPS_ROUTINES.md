...
> **Automation (updated 2026-07-04 evening — launch-sprint consolidation):** the scheduled-task suite, which had grown to 16 executive routines, was cut back to 4 refocused on the sprint to live production: `morning-compliance-defense` = **Morning Launch Gate** 7:51 AM (test/typecheck/invariant gates + security quick-scan + AAN seams; absorbed the retired security-posture, risk-governance, code-checkup, and bridge-bind routines), `midday-lender-liquidity` = **Lender Delivery Gate** 12:31 PM (Routine 2 below), `evening-economics-gtm` = **Evening Launch Triage** 6:38 PM (report triage + maintains the 🚀 Launch sprint section of CTO_ROADMAP.md + docs-only publish of local main; margin rehearsal suspended until a real rate sheet or F11 lands), and `weekly-vendor-procurement` Mondays 9:33 AM (absorbed the daily vendor-portal sweep). **Routine 3 (chaos drills) is suspended until real traffic/F3** — the kill switch is verified (#27) and the medical-debt drill is F3-gated; the other 11 retired routines are disabled with `[RETIRED]` markers, prompts preserved for post-launch revival. Each live routine emits a dated report into `kb/founder-routines/`; human-only steps (portal logins, lender calls) are emitted as checklists, not simulated. This doc remains the corrected doctrine.
