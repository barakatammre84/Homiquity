...
**Read by every `journey-walker-*` agent. Not restated in them — read this file, not a copy of it.**
Added 2026-08-23 to end a real drift: the server-truthfulness checklist and the full walk
procedure/output contract were independently duplicated across `SKILL.md` and all five walker
agents, and the rotation ledger had already gone stale relative to the persona count while nobody
noticed. This file is the one place that content lives now, following the same pattern
`.claude/agents/_OWNER_RAILS.md` already uses for the 41 `hq-*-owner` agents ("read, not copied — a
rail changes in one place and there is nothing to drift").

**Precedence.** This file wins over anything a walker agent restates from it — if a walker agent's
prose disagrees with this file, this file is right and the agent file is a bug to report. For
persona-specific facts (which route, which account, which threshold, which `file:line` evidence),
[`knowledge-base/feature-review/JOURNEYS.md`](../../knowledge-base/feature-review/JOURNEYS.md) wins
over this file — it is the single source of truth for personas, routes, accounts and thresholds.
The code wins over both. A walker that finds this file, its charter and the code disagreeing reports
the disagreement rather than picking one silently.

## Ground rules (binding on every walker)
...
- **J1 — Seams, not surfaces.** Every surface already has an owner: a defect wholly inside one
  ...
  route. Anything else is a `HANDOFF` line naming the owner — **no id is minted** (CHARTER §7). **Name
  a file's actual owner**, not a category: `knowledge-base/handbook/FEATURE_MAP.md` maps every path
  to one `hq-*-owner` agent, and that agent implements. A hand-off that names an owner becomes work; a
...
- **J3 — Know what you are pointed at.** Local only, and **default to `http://localhost:5001`**
  ...
  *previous* container serving, so what renders there may not be the code under review. A local
  ...
  code, say so in the report rather than attributing findings to today's code. **And a long-lived
  listener is stale on the server side even when it is the right checkout**: `pnpm dev` is
  `tsx server/index-dev.ts` with **no watch flag**, so the server half is frozen at process start
  while the client is Vite-transformed per request and stays current. Compare the process start time
  against `git log -1 --format=%cd` and against the mtimes of `server/**` — if server files are newer
  than the process, **say so and attribute server-side findings to the older code**, or restart
  before walking. Verified 2026-08-19: a 49-minute-old listener was serving the current client over
  server code that predated two merged commits.
- **J4 — Your own account, and only yours.** The exact convention (email prefix, starting/ending
  role, thresholds to cross) is in your walker agent and in `JOURNEYS.md` for your journey number —
  they must agree; if they don't, `JOURNEYS.md` wins and the disagreement is a finding about this
  corpus. Use only obviously-fake PII (test SSNs matching the convention already in `tests/`).
  **Never touch a row you did not create; never run destructive SQL; never `pnpm db:push`.** The dev
  database is shared.
...
## Walk procedure (the shared skeleton — your agent file inserts its own step 5)

1. **Charter first.** Read your journey charter (`JOURNEYS.md` §N) and restate it as
...
5. **Your walker agent's unique step.** Every journey but the sandbox one crosses the promotion at
   this point — the shared shape:
   ...
   Your agent file then names what to do *after* crossing it — that part is genuinely
   persona-specific (a branch to prove carried, a threshold to cross, two passes to diff, a sandbox
   to map) and is not generalized here.
...
## Output (the shared frame — your agent file names its own middle block)

Return (as your final message) a structured walk — no prose preamble:

```
JOURNEY: <N. name, from JOURNEYS.md>
SERVER: <base url> (health: ok/fail · checkout: <path> · started: <time> · prelaunch: open/gated)
ACCOUNT: <per your agent file's ACCOUNT rule> (start role → end role)
ROUTE:
...
<YOUR AGENT'S PERSONA-SPECIFIC BLOCK HERE, if it has one — named in your agent file>
PROMISES:
...
The `aspiring-owner` walker never crosses the promotion (it must never apply), so it omits
`TRANSITIONS` and substitutes a `SANDBOX FLOOR` block in the same position — its agent file says so.
