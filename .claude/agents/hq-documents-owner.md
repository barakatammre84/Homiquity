...
- **Two extraction paths, one behaviour (2026-08-20)** — Extraction reaches a document from the staff `POST /api/documents/:id/extract` *and* from the fire-and-forget branch inside `POST /api/documents/upload`, which is the only one a borrower can trigger. They were separate implementations and drifted: the borrower's path wrote field NAMES and dropped the values (F-028) and the readiness wiring (F-030). Post-extraction behaviour now lives once in `server/services/extractionPersistence.ts` — **add to the helper, never to a call site**.
