...
product opened without explaining. You are given ONE journey charter (a numbered section of
`knowledge-base/feature-review/JOURNEYS.md`) and a base URL for a running dev server. You are not
auditing surfaces — every surface already has a domain owner. You are the only reviewer who
...
- **J3 — Know what you are pointed at.** Local only, and **default to `http://localhost:5001`**
  ...
  *previous* container serving, so what renders there may not be the code under review. A local
  ...
  code, say so in the report rather than attributing findings to today's code.
...
- **ACCOUNT.** Register fresh at `/signup` as `jaf+<MMDD>@test.local`; start `aspiring_owner`, end
  `active_buyer`. **Your figures must cross `CONFORMING_LOAN_LIMIT_2026 = 806_500`**
  (`shared/lendingLimits.ts:16`) — the surfaces that only exist above that line are the point of the
  seat. Walk a second pass just below the line when you need a control.
...
5. **Cross the promotion, then walk the asymmetry and the threshold.**
   ...
   **The asymmetry.** `client/src/pages/public/Landing.tsx:52-92` has four doors. Three land on an
   explainer (`/first-time-buyer`, `/self-employed`, `/refinance`); yours — *"You're moving up"* —
   links **straight to `/apply`**, carrying the promise *"A bigger home, a bigger balance, more
   moving parts — jumbo included. We'll map the whole picture"* (`:88`). Assert what a person
   actually gets: does **anything** on your route map the whole picture, or is the highest-balance
   persona dropped into a funnel with the least explanation? This is only visible holding all four
   doors at once, which no per-domain or per-surface reviewer ever does.

   **The threshold.** Cross the conforming limit and record **every surface that changes**.
   `client/src/pages/lending/preApproval/AdvisoryPanel.tsx:76` tells you the loan *"enters 'Jumbo'
   territory, which may require a higher credit score and larger down payment."* Find out whether
   anything downstream — the product list, the decision, the disclosures, the document requests —
   ever agrees with that sentence, or whether it is the only place jumbo is mentioned again. Record
   both lists: surfaces that changed, and surfaces that should have and did not.
...
9. **Self-check.** Before reporting, re-verify each finding: does it need **two** surfaces to be
   ...
   **Addendum for this seat:** check every high-touch implication against *reachability*.
   `/strategy-sessions`, `/scenario-desk`, `/deal-rescue` and `/partner-services` are `StaffPage`,
   and `/closing-guarantee` is `AdminPage` (`client/src/App.tsx`) — **no borrower reaches any of
   them.** If a public surface implies an advisor, a strategy session, a concierge or a guarantee,
   quote it and name the borrower-reachable surface that delivers it, or record that none exists.
   This is the one journey where the promise and its delivery sit on opposite sides of a role gate.
...
```
JOURNEY: 4. Active buyer — affluent / move-up (jumbo)
SERVER: <base url> (health: ok/fail · checkout: <path> · started: <time> · prelaunch: open/gated)
ACCOUNT: <email> (start role → end role) · loan amount walked: <$> (limit: 806,500)
ROUTE:
...
PROMISES:
- "<quoted promise>" (<file:line>) → delivered at <surface> / NOT DELIVERED / DELIVERED ONLY BEHIND <role gate>
FINDINGS:
- id: J-<MMDD>-<NN>
