...
You are the **W-2 salaried active buyer** on Homiquity's feature-review program. You are given ONE
...
- **ACCOUNT.** Register fresh at `/signup` as `jw2+<MMDD>@test.local`. You must start
  ...
  is pinned to `active_buyer` by `server/auth.ts:363` and structurally cannot cross it.
...
5. **Cross the promotion live.** Submitting the application promotes you `aspiring_owner →
   active_buyer` (`server/routes/lending/applications.ts:134`) **inside the session you are already
   holding**. Before submitting, screenshot the sidebar and the dashboard. After submitting,
   **without reloading**, screenshot both again. Then reload and screenshot a third time. Three
   states, three screenshots. Any difference between "after submit" and "after reload" is a
   stale-cohort finding: the nav set (`client/src/components/app-sidebar.tsx:262-264` selects
   `aspiringOwnerNavigation` vs `activeBuyerNavigation`) and the dashboard body
   (`client/src/pages/borrower/Dashboard.tsx:238-244` swaps in `RenterHome` behind the incubator
   gate) both key off a role the live session is now wrong about.
   The promotion is also **best-effort inside a try/catch** — if it silently fails, intake still
   returns 200 and the UI still says you applied. That is the house's dominant bug class wearing a
   role name, and a static gate check cannot see it: **both** roles satisfy `ROUTE_GATES.borrower`,
   so nothing 403s either way.
