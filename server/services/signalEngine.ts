 ...
 */
/**
 * A file's document requirements can GROW after intake: the derivation reads
 * the SET of income sources (server/pipelineEngine.ts), so a borrower who adds
 * a second employer or an other-income row on a later URLA save legitimately
 * comes to owe documents nobody asked for at intake.
 *
 * That is correct — we were failing to ask — but it can put new blockers on a
 * file a loan officer believed was clear, so it is surfaced rather than left to
 * be discovered at submission.
 *
 * Derived, not pushed: `generateConditionsFromRequirements` writes one
 * auto-generated `DOC_REQ_<TYPE>` condition per requirement in a single pass,
 * so the intake batch shares a timestamp cluster and anything materially later
 * is a widening. Keeps this feed a pure read of state the engines already
 * maintain, with no new column and no new write path.
 */
export const WIDENING_GRACE_MINUTES = 10;

export interface WidenedConditionRow {
  applicationId: string;
  title: string;
  createdAt: Date | string | null;
}

export function planRequirementsWidenedSignals(
  rows: WidenedConditionRow[],
): { applicationId: string; titles: string[]; count: number }[] {
  const byApp = new Map<string, { at: number; title: string }[]>();
  for (const r of rows) {
    if (!r.createdAt) continue; // no timestamp ⇒ cannot place it either side of intake
    const at = new Date(r.createdAt).getTime();
    if (Number.isNaN(at)) continue;
    const list = byApp.get(r.applicationId) ?? [];
    list.push({ at, title: r.title });
    byApp.set(r.applicationId, list);
  }

  const out: { applicationId: string; titles: string[]; count: number }[] = [];
  for (const [applicationId, list] of byApp) {
    const earliest = Math.min(...list.map((x) => x.at));
    const cutoff = earliest + WIDENING_GRACE_MINUTES * 60_000;
    const later = list
    ...
    out.push({
      applicationId,
      titles: later.map((x) => x.title),
      count: later.length,
    });
  }
  return out.sort((a, b) => b.count - a.count || a.applicationId.localeCompare(b.applicationId));
}
...
export async function buildStaffSignals(limit = 30, scope?: StaffSignalScope): Promise<StaffSignal[]> {
  ...
  const [conditionRows, widenedRows, reviewDocRows, docRows, userRows] = await Promise.all([
    db
      .select({
    ...
    // Auto-generated document requirements, for the widened-after-intake signal.
    // Outstanding only: one the borrower has already satisfied needs no attention.
    db
      .select({
        applicationId: loanConditions.applicationId,
        title: loanConditions.title,
        createdAt: loanConditions.createdAt,
      })
      .from(loanConditions)
      .where(
        and(
          inArray(loanConditions.applicationId, appIds),
          sql`${loanConditions.isAutoGenerated} = true`,
          sql`${loanConditions.sourceRule} LIKE 'DOC_REQ_%'`,
          sql`${loanConditions.status} = 'outstanding'`,
        ),
      ),
  ...
  // 2 — requirement set grew after intake (a second income source appeared)
  for (const group of planRequirementsWidenedSignals(widenedRows)) {
    const shown = group.titles.slice(0, 3).join("; ");
    const rest = group.count - Math.min(3, group.count);
    signals.push({
      type: "requirements_widened",
      priority: 2,
      applicationId: group.applicationId,
      borrowerName: borrower(group.applicationId),
      title: `${group.count} document requirement${group.count === 1 ? "" : "s"} added after intake`,
      detail: rest > 0 ? `${shown} (+${rest} more)` : shown,
    });
  }
