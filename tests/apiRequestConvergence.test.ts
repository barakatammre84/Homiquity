...
const ALLOWED_RAW_FETCH: Record<string, string> = {
  ...
  //
  // 🚨 THE FOUR CAPTURE FORMS ARE GONE FROM THIS LIST, and the reason is worth
  // keeping. They were exempted as "public POST" — a statement about the 401
  // redirect, which was true. But the exemption silently covered a SECOND
  // question it never asked: whether the site checks the response at all.
  //
  // EmailCaptureModal and Waitlist did not. `await` on the browser primitive
  // only rejects on a NETWORK error, so a 400 (Zod-invalid address) or a 429
  // (5 per 15 min per IP, server/app.ts emailCaptureLimiter — one shared office
  // or CGNAT mobile IP) resolved normally and both surfaces rendered SUCCESS.
  // Waitlist is "/" while PRELAUNCH_GATED, so the primary pre-launch conversion
  // surface told people they were on the list and dropped them. PartnerWaitlist
  // and RentReporting did check, but threw a bare Error that discarded the
  // server's message envelope.
  //
  // The "each owns error copy apiRequest would replace" rationale was already
  // walked back once in this file — see the ApplyInvite note below:
  // friendlyApiError parses the "<status>: <body>" shape and extracts the
