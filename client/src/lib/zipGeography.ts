/**
 * ZIP-code geography for the public rates surface — THE derivation of "which
 * state is this ZIP in" and "what does a home cost there".
 *
 * WHY THIS FILE EXISTS. This lived in SEVEN places, in two mutually
 * inconsistent implementations:
 *
 *   - components/RatePageHeader.tsx held the ZIP3 table below (908 prefixes,
 *     all 50 states + DC + PR) and used it to pick the property value it
 *     DISPLAYED.
 *   - Each of the six pages under pages/rates/ carried its own copy of a
 *     ten-branch numeric-range stub covering CA, NY, TX, FL, IL, PA, OH, GA,
 *     NC and MI — and THAT one fed the query key, i.e. the `?state=` the
 *     server actually filters rates by
 *     (storage.getMortgageRatesForLocation, server/routes/admin.ts).
 *
 * So for a visitor in any of the other forty states the header printed a
 * confident state-average home value while the rate request went out with no
 * state at all. Two derivations of one fact, disagreeing, on a public
 * rate-advertising surface — and six copies of the wrong one, so any fix
 * landed in a single file and left five behind.
 *
 * One table, one function, no React. Keep it that way: anything that needs a
 * state or a price from a ZIP imports from here.
 */

/**
 * Fallback when a ZIP resolves to no state (an unassigned prefix, or fewer
 * than three digits typed). Deliberately the SAME value the header used
 * before, so an unresolvable ZIP renders exactly as it did.
 */
export const DEFAULT_HOME_PRICE = 350000;

/** Typical home price by state. Display-only — never an underwriting input. */
const STATE_AVERAGE_HOME_PRICES: Record<string, number> = {
...
/** First three ZIP digits -> USPS state code. */
...
/**
 * The state a ZIP falls in, or `undefined` when it resolves to none.
 *
 * `undefined` (not `null`) because the value goes straight into a query-key
 * params object, and buildQueryUrl drops undefined/null/"" alike — so an
 * unresolved ZIP produces `/api/mortgage-rates?zipcode=...` with no `state`
 * param rather than `state=null`.
 */
export function stateFromZip(zip: string | undefined | null): string | undefined {
  if (!zip || zip.length < 3) return undefined;
  return ZIP3_TO_STATE[zip.substring(0, 3)];
}

/** The typical home price where this ZIP is, or DEFAULT_HOME_PRICE. */
export function averageHomePrice(zip: string | undefined | null): number {
  const state = stateFromZip(zip);
  return (state && STATE_AVERAGE_HOME_PRICES[state]) || DEFAULT_HOME_PRICE;
}
