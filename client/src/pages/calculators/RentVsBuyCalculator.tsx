...
export default function RentVsBuyCalculator() {
  ...
  const [inputs, setInputs] = useState<RentVsBuyInputs>(rentVsBuyDefaultInputs);

  usePageView("/calculators/rent-vs-buy");
  const trackActivity = useTrackActivity();
  const trackedRef = useRef(false);

  const results = useMemo(() => calculateRentVsBuyResults(inputs), [inputs]);
  ...
  const saveResultsMutation = useMutation({
    mutationFn: async (data: { inputs: RentVsBuyInputs; results: RentVsBuyResults }) => {
  ...
  const updateInput = (field: keyof RentVsBuyInputs, value: number) => {
