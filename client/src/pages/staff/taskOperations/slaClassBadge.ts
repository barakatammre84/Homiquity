 ...
 *
 * So the stored colorCode maps to the design system's semantic tokens — the same
 * FAMILIES SlaHeatmap uses for its bars (destructive / warning / muted), in the
 * `-subtle` tints that suit a badge's tinted background rather than the solid
 * fills a bar wants. Six palette values compress onto four severity tiers, so
 * S0+S1 (immediate + critical, both file-blocking) share the destructive tier and
