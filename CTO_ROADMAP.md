...
- [~] **1. Fix file uploads in production.** Uploaded documents currently go to temporary disk on Vercel and vanish. Configure the Google Cloud Storage bucket + credentials, make the presigned-URL flow (`/api/uploads/request-url`) the only path, and delete the multer disk-storage path in `server/routes/utils.ts`. Acceptance: upload a document in production, redeploy, document is still there. *(Code done 2026-07-03, prod acceptance pending your GCS env vars. `/api/documents/upload` is now a JSON metadata-registration step: the client PUTs bytes straight to storage via the presigned flow, then registers by `objectPath` — so `storagePath` is a persistent `/objects/…` path, never the ephemeral disk path that was the bug. All 3 client callers (Documents, Tasks, TaskDetail) converted to the two-step flow; the endpoint verifies the object actually exists before recording it. Multer disk storage is KEPT only for the ephemeral extraction endpoints (tax-return/paystub/bank-statement/lease — they OCR then delete, not persist), so `utils.ts` `upload` stays. Local dev without GCS uses a filesystem fallback (`localObjectStorage.ts`) so the same flow is testable — verified end-to-end locally (round-trip bytes identical; integrity/traversal/auth/magic-byte guards all pass). **Remaining = your ops action:** set `GCS_SERVICE_ACCOUNT_KEY`, `PRIVATE_OBJECT_DIR`, `PUBLIC_OBJECT_SEARCH_PATHS` in Vercel, then run the prod acceptance test. Note: direct-to-GCS also sidesteps Vercel's 4.5 MB function-body limit that the old multipart path hit.)*
