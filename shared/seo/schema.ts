...
/**
 * The default social card. Generated by `pnpm brand:og` — never hand-edited.
 *
 * The filename changed from og-default.png in 2026-08 rather than the bytes being
 * replaced, because express.static serves dist/public with `maxAge: "1y",
 * immutable` and Vite copies publicDir verbatim WITHOUT content-hashing. Swapping
 * the bytes of a name that browsers and scraper caches already hold as immutable
 * reaches nobody who has seen the site, and no query string busts a social
 * scraper's cache. A future change to this card needs a new filename too.
 *
 * og-default.png still exists and now carries the SAME bytes — links shared
 * before the rename name it, and refreshing it stops NEW fetches of the old URL
 * from serving the retired green card.
 *
 * This is the ONLY declaration. client/src/components/SEOHead.tsx used to
 * redeclare it independently; nothing tested that the two agreed (canonicalHost
 * pins the HOST, not the filename), so they were free to drift.
 */
