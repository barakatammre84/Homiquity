...
|---|---|---|---|
| founder-directed session (UI-overhaul prototype, PR #558 stage 2) | `client/src/components/patterns/**` (11 new components + colocated tests) · `pages/staff/DesignPrototype.tsx` · `lib/dualUnitMath.ts` · `App.tsx` (one lazy route) · `index.css` (`--muted-foreground` → green-grey, DESIGN-STANDARD §4) | 2026-08-18 | **shipped to review** — rides [PR #558](https://github.com/barakatammre84/Homiquity/pull/558) on top of the design-standard landing. The §9 type-encoded invariants are test-pinned (frozen denominators, derived master checkbox, no pre-ticked consent, required source/whyWeAsk/scope). `ProvenanceBadge` derives from `shared/dataProvenance.ts` (no second enum, per the 08-18 audit §2b.5). Gates: `check` 0 · client 93 files/608 green · tokens at baseline · querykeys OK · bundle re-baselined 522,536→522,832 (+296 B = the one new lazy route registration; all component code is in the lazy chunk). Peers: `components/patterns/**` is now shared ground — extend it rather than hand-rolling (root `CLAUDE.md` hard rule). |
