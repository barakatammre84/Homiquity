...
|---|---|---|---|---|---|
| founder-directed session (borrower task/progress honesty — same PR #563) | `client/src/pages/borrower/Tasks.tsx` (+new test) · `client/src/pages/borrower/Dashboard.tsx` (readiness label only) · `client/src/components/patterns/TaskProgress.tsx` | primary checkout | `claude/file-implementation-naijtt` | 2026-08-18T19:05Z | DESIGN_SYSTEM §13 Agreement on the borrower task surfaces: Tasks' "Your Progress" recomputes its denominator from live data every render, so an LO assigning a task mid-session makes the borrower's percentage run BACKWARDS (teardown §6.2's exact defect) → migrate to `TaskProgress` (frozen denominator, labeled); harden `useTaskProgress` to freeze on the first meaningful total so a consumer rendering during load can't pin 0; Tasks' hand-rolled zero state → scoped `EmptyState`. Dashboard's Loan Progress card renders a bare weighted-heuristic `readiness` % with no statement of what it measures (§13/§8 unlabeled progress) → label it. **JourneyTracker deliberately NOT replaced** — §15 names Dashboard.tsx as the standard's own reference surface; swapping in `LoanTracker` would regress the exemplar. Not touched: `borrowerDashboard/model.ts` weights, task engine, `text-status-*` hex tokens (that's `guard:ui` rawHexLiterals' lane, 33 sites). Founder-directed. Releases in the same PR. |
| founder-directed session (pattern library + surface migrations — the #558 follow-up PR) | `client/src/pages/borrower/urla/DeclarationsSection.tsx` (+new test) · `client/src/pages/borrower/documents/UploadedDocumentsTable.tsx` (+new test) · `client/src/pages/lending/ApplicationSummary.tsx` (+new test) · `client/src/components/patterns/**` (+tests) · `pages/staff/DesignPrototype.tsx` · `lib/dualUnitMath.ts` · `index.css` (`--muted-foreground`) · `App.tsx` (one lazy route) | primary checkout | `claude/file-implementation-naijtt` | 2026-08-18T18:30Z | Re-land #558's recorded surviving work reconciled to `DESIGN_SYSTEM.md` (#560): the pattern library + three surface migrations (URLA declarations wall → `DeclarationsGroup`, escape hatch on the E–K history block only; uploaded-docs table → `DocumentList`; application summary → `SummarySection`/`ProvenanceBadge`/`DualUnitTable`), `MilestoneBar`/`OptionalUpload`. **Not carried:** `docs/DESIGN-STANDARD.md`, the root `CLAUDE.md` rewrite, and history edits (archive/logs/ledger reverted to main — #559 ratchet doctrine). **URLAForm.tsx itself is NOT touched** (URLA_FORM_REFACTOR_TRAP respected — only the extracted section file changes). `Documents.tsx` untouched (the §9-reviewed upload path stays; only the table component's internals change). No overlap with open #560 (funnel files) or `claude/ux38-batch1-nested-controls` (Navigation). Founder-directed. Releases in the same PR. |
