...
|---|---|---|---|
| founder-directed session 2026-08-18 (the three decisions) | the six previously-invisible routine definitions under `.claude/skills/`, `knowledge-base/routines/{CHARTER,registry.json}`, `primary-engineer/SKILL.md` R5 | 2026-08-18 | **shipped to the same branch.** Founder took all three open decisions. (1) **The six definitions are committed** — reconstructed from CHARTER + each routine's own reports, each carrying a provenance header; the private copies still exist, so `registry.json` flags `schedulerRepoint.required` per routine and `guard:routines` warns until the founder repoints each task and merges in any rail the private copy carries. (2) §6's `data/regulatory/**` carve-out **confirmed**. (3) The `package.json` rail now names the **dependency blocks** as the never; a `scripts` entry wiring a same-PR guard is L2. **Refactor Radar's R4 is unchanged and still forbids `package.json` entirely** — peers, do not read the §6 change as relaxing radar. |
| founder-directed session 2026-08-18 (routine-suite audit) | `knowledge-base/routines/**`, `scripts/routine-registry-guard.cjs` (new), `package.json`/`checkup.sh`/`ci.yml`/`doc-freshness.yml` wiring, `.claude/settings.json` + `.claude/hooks/` (new), `tests/routineGovernance.test.ts` (new) | 2026-08-18 | **pushed to `claude/routines-code-quality-review-snqxol`, no PR opened** (not asked for). Audit of both fleets in [logs/2026-08-18-routine-suite-audit.md](../logs/2026-08-18-routine-suite-audit.md). Ships `pnpm guard:routines` (required gate) + `registry.json`, CHARTER §8's hard-nevers as PreToolUse hooks over Bash **and** the GitHub MCP merge tools, CHARTER §2/§3/§6/§11 corrections and a new §12 assigning five ownerless controls. **Heads-up for peers:** two shared-file hazards touched — `vitest.config.ts`'s `include:` array (one line) and `scripts/ui-standard-baseline.json` (`nestedInteractive` 122 → 44, a real re-measure after #560 re-opened what #561 had closed). Resolve both additively. **Founder decisions left open, not taken:** commit the six laptop-only routine definitions (Evening Triage first); confirm the §6 `data/regulatory/**` narrowing; resolve the `package.json` deviation this PR declares rather than assumes. |
| founder session (interactive) 2026-08-18 | pre-approval funnel capture path (`claude/routines-operating-cadence-olj41t`) | 2026-08-18 | **shipped** — 320px grids, step-input rung, AdvisoryPanel reaches mobile, full-width CTA, canonical gutter. Claimed 18:05, released 18:15. `FunnelChrome.tsx` claimed but NOT edited: the §12.2 disclosure-placement question is compliance-bearing and went to the founder instead. |
