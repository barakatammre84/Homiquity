...
- **Ship confirmation:** `verify-deploy` polls `https://homiquity-production.up.railway.app/api/health` (the Railway origin — pinned by `tests/ciTriggers.test.ts`; never `www`) until its
