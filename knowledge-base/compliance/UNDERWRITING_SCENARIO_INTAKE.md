# Underwriting scenarios — intake queue

> Split out of [UNDERWRITING_SCENARIOS.md](./UNDERWRITING_SCENARIOS.md) on 2026-08-06. That file was
> 1,166 lines carrying two different lifecycles: a **registry of shipped scenarios** (stable, cited,
> catalog-synced) and this **intake queue** (churning, unadjudicated). ~85% of the bytes were this
> half, which buried the registry. Nothing was rewritten — every entry below is verbatim.
>
> **Nothing here is implemented.** An item leaves this file only by acquiring a guideline citation
> and a deterministic rule, at which point it moves to the registry's `## Implemented` section and
> `server/services/scenarioCatalog.ts` **in the same commit** (enforced by
> `tests/scenarioCatalog.test.ts`). Items without a citation stay in Needs Clarification with the
> specific question that would resolve them — they are never implemented on judgement alone.
>
> Operating instructions: [SCENARIO_ARCHITECT.md](./SCENARIO_ARCHITECT.md).
