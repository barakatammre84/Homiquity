...
- **Highest-risk code areas:** `server/routes/borrower/`, `server/routes/lending/`, `server/routes/compliance.ts`, `server/routes/documents.ts`, `server/routes/task-engine.ts`, `server/routes/underwriting/`, `server/routes/underwriting-rules.ts`, `server/storage/`
- **Newly validated hot spots:** `server/routes/agent-broker/`, `server/routes/staff-invites.ts`, `server/routes/intelligence.ts`, `server/routes/data-intelligence.ts`, `server/auth.ts`, `server/socialAuth.ts`, `server/integrations/auth/` (the Replit auth integration was removed 2026-07-02), onboarding verification routes in `server/routes/borrower/`, shared pipeline views such as `/api/pipeline/queue`, compliance-wide views such as `/api/compliance/dashboard` and `/api/credit/archive-eligible`, document list/download/upload routes in `server/routes/borrower/`, `server/routes/documents.ts`, and `server/routes/lending/`, document-package mutation routes in `server/routes/borrower/` (especially `POST /api/document-packages/:packageId/items`), document-request mutations inside atomic upload and human-review routes, file/financial review transactions in `server/routes/file-review.ts` and `server/routes/financial-review.ts`, generic staff status mutations in `server/routes/lending/`, closing-guarantee routes in `server/routes/borrower/`, homeowner post-close routes such as `/api/homeowner/equity/:profileId` and `/api/homeowner/equity`, the global API response logger in `server/app.ts`, deal-team mutation routes, and endpoints that accept raw `applicationId`, `taskId`, `documentId`, `letterId`, invite codes, verification identifiers, snapshot IDs, profile IDs, message IDs, or user IDs
