...
| 🟡 partial | 15 | Computed or partially encoded, but not binding on the decision, or missing a documented leg. |
| ❌ absent | 24 | The Guide states a rule that binds us and no code implements it. |
...
| `B2-1.2-01` | Loan-to-Value (LTV) Ratios | 🟡 partial | server/underwritingEngine.ts resolves the CONVENTIONAL_MAX_LTV matrix on the decision path — but this section states no maximum LTV to conform TO. Its two tables define how the ratio is CALCULATED (purchase: original loan amount over the lower of sales price or appraised value; refinance: over the current appraised value; plus co-op and financed-MI variants) and it then says 'Refer to the Eligibility Matrix for maximum allowable LTV ratios'. VERIFIED ON PDF sheets 179-181 by extracting the tables, because the flattened text makes them look like they might carry ceilings. We do not hold the Eligibility Matrix (gap G-14), so our ceilings are platform-conservative, not agency-verified. Downgraded from implemented on 2026-08-23; the calculation half conforms, the maximums half is unverifiable in-repo. See correction C-13. |
...
| `B2-3-03` | Special Property Eligibility and Underwriting Considerations: Leasehold Estates | ❌ absent | Leasehold Estates, revised 08-05-2026 (co-op ground-lease renewal, UAD 3.6 Policy Supplement). No leasehold concept exists: grep -rIn -i 'leasehold' over server/ shared/ client/src returns 0. Ground rent is also one of the four PITIA components B3-6-03 requires and we do not capture (gap G-10), so a leasehold file would be qualified on an incomplete housing expense. Nothing to conflict with today because no code branches on estate type. |
...
| `B4-2.1-01` | General Information on Project Standards | ❌ absent | General Information on Project Standards, revised 08-05-2026. No project-standards code exists anywhere: grep for project review, PERS (word-bounded), single-entity and master-policy terms all return 0 across server/ shared/ client/src. The funnel collects property_type=condo and nothing downstream branches on it for eligibility, so a fully qualified borrower is never declined for the building. Re-verified 2026-08-23 as part of dispositioning gap G-6 (the sections revised in this edition). |
| `B4-2.1-02` | Waiver of Project Review | ❌ absent | Waiver of Project Review. No project-review concept exists (grep 'project review'/'projectReview' -> 0), so neither the waiver nor the review it waives is modelled. Referenced indirectly by B7-4-02's fidelity waiver, which we also do not implement. |
| `B4-2.1-03` | Ineligible Projects | ❌ absent | Ineligible Projects, revised 08-05-2026. Single-entity ownership caps VERIFIED ON PDF sheets 656 and 663 (table flattens in extracted text): more than 2 units for projects of 5-10 units in a master association, more than 2 units for 11-20 units, more than 20% for 21 or more; co-op equivalent 20% of stock/shares. Units under any rental or lease arrangement, including lease-purchase and rent-to-own, count toward it. No project eligibility code exists at all; condo files are decisioned with no project test. |
| `B4-2.2-01` | Full Review Process | ❌ absent | Full Review Process. No review-tier concept (Limited vs Full vs PERS) exists anywhere in the tree. |
| `B4-2.2-02` | Full Review: Additional Eligibility Requirements for Units in New and Newly Converted Condo Projects | ❌ absent | Full Review: additional requirements for new and newly converted condo projects. Not modelled; the funnel's condo answer reaches no project logic. |
| `B4-2.2-04` | Project Eligibility Review Service (PERS) | ❌ absent | Project Eligibility Review Service (PERS). grep -E '\bPERS\b' over server/ shared/ client/src -> 0 (an unbounded grep matches 'person' and is the wrong evidence). Not modelled. |
| `B4-2.3-01` | Eligibility Requirements for Units in PUD Projects | ❌ absent | Eligibility Requirements for Units in PUD Projects. Not modelled; property_type carries PUD but nothing branches on it for eligibility. |
| `B5-3.2-05` | HomeStyle Renovation Mortgages: Completion Certification | ❌ absent | HomeStyle Renovation completion certification. We do not originate HomeStyle. Note the asymmetry: shared/fannieMae/specialFeatureCodes.ts carries SFC 215/279 (Renovation with/without recourse) and 375/773 (Energy), so the DELIVERY codes exist while the eligibility rules behind them do not. All are autoDerived:false, so nothing can assert one on its own. |
| `B5-3.2-06` | HomeStyle Renovation: Renovation Contract, Renovation Loan Agreement, and Lien Waiver | ❌ absent | HomeStyle Renovation contract, loan agreement and lien waiver. Dated 12/11/2024, so it predates this edition; it appears in revised-sections.tsv because the release's highlight annotations mark the page, not because the section text changed. Not modelled, same as B5-3.2-05. |
| `B5-3.3-01` | HomeStyle Refresh for Improvements on Existing Properties | ❌ absent | HomeStyle Refresh. Not modelled; SFC 892 (HomeStyle Refresh) exists in the delivery catalog, autoDerived:false, with no eligibility rule behind it. |
| `B5-5.2-02` | Loans with Resale Restrictions: Eligibility, Collateral and Delivery Requirements | ❌ absent | Loans with Resale Restrictions. Not modelled. SFC 630/631 (restrictions terminate/survive at foreclosure) exist in the delivery catalog with no rule behind them, both autoDerived:false. Relevant to the DPA lane: the Illinois programs we surface are subordinate liens (gap G-21), and resale-restricted affordable programs are the neighbouring class. |
...
| `B7-3-02` | Property Insurance Requirements for One-to Four-Unit Properties | ❌ absent | Property insurance requirements for one- to four-unit properties, revised 08-05-2026. Still absent: no insurance-adequacy check, no coverage-amount test, no evidence-of-insurance document type. Each individual peril deductible must not exceed 5% of the coverage amount (PDF sheet 875). Re-verified 2026-08-23 while dispositioning gap G-6. |
| `B7-3-03` | Master Property Insurance Requirements for Project Developments | ❌ absent | Master property insurance for project developments, revised 08-05-2026. Maximum deductible VERIFIED ON PDF sheets 879-880: 5% of the master coverage amount per occurrence, and $50,000 per unit where the policy carries a per-unit deductible (a deductible buy-back policy may be used to meet it). Boiler/machinery coverage required where there is central heating or cooling. No insurance-adequacy code exists; grep 'master policy\|master insurance' -> 0. |
| `B7-3-04` | Property Insurance Requirements for Individual Units in a Project Development | ❌ absent | Property insurance for individual units in a project development (walls-in / HO-6). Not modelled. The only walls-in reference in the tree is a borrower-education glossary definition, which states no requirement. |
| `B7-3-05` | Additional Insurance Requirements | ❌ absent | Additional insurance requirements. Dated 12/14/2022 — predates this edition, present in revised-sections.tsv via the release's highlight annotations rather than a text change. Not modelled. |
| `B7-3-07` | Evidence of Property Insurance | ❌ absent | Evidence of property insurance. Not modelled: no insurance document type, no evidence check, and the document checklist has no insurance binder requirement. |
| `B7-4-01` | General Liability Insurance Requirements for Project Developments | ❌ absent | General liability insurance for project developments. grep -i 'liability insurance' over server/ shared/ client/src -> 0. Not modelled. |
| `B7-4-02` | Fidelity/Crime Insurance Requirements for Project Developments | ❌ absent | Fidelity/crime insurance for project developments, revised 08-05-2026. Required for all condo and co-op projects except those qualifying for a waiver of project review (B4-2.1-02), projects of 20 units or less, or projects needing $5,000 or less of coverage — VERIFIED against PDF sheet 901. Not modelled; the only 'fidelity' string in the tree is unrelated prose in a funnel component. |
...
| `E-2-04` | Signature Requirements for Mortgages to Inter Vivos Revocable Trusts | ❌ absent | Signature requirements for mortgages to inter vivos revocable trusts. Not modelled: no trust borrower type, no trustee signature handling. SFC 168 (Inter Vivos Revocable Trust) exists in the delivery catalog, autoDerived:false, with no origination rule behind it. Note E- has no not_applicable prefix: these are seller/servicer AND originator-facing signature mechanics, so 'absent' is the honest status rather than n/a. |
