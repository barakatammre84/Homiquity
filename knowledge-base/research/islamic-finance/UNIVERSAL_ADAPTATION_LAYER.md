...
> **Status update (2026-07-11) — partially superseded by the UAL program.** The founder directed
> productization via [`specs/UNIVERSAL_ADAPTATION_LAYER_PROGRAM.md`](../../specs/UNIVERSAL_ADAPTATION_LAYER_PROGRAM.md)
> prompt **P7**: the plan approval IS the cut-line decision for the **funder-agnostic build items**
> (halal-need intake signal, the §3 translation calculators beside `apr.ts`, catalog extensibility).
> The two founder calls (§ LENDER_CHANNEL_VALIDATION) and the §5 counsel register remain the
> **hard gates** for any funder wiring, marketing, or origination. The broker-triage firewall below
> (no held title / SPV / own Shariah cert) is unchanged and binding. The original status banner is
> kept for the still-gated remainder:
>
