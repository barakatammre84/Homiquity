...
| 2 | Intake → AUS → lender package → wholesale submission | FULLY — AUS UI trigger + re-wired Submit-to-lender shipped in #135 (was PARTIAL, F-003) | **2026-08-05 verifier — full run incl. status machine to funded** · **2026-08-23 — full LIVE HTTP re-run against `:5001`** (PID 31999, `lsof` cwd = the repo, `/api/health` `commit:null` = the local signature; seeded `seedDemoFile.ts` file; all 8 steps plus the machine to `funded` plus a pipeline-side extension; evidence in [journey-walks/2026-08-23-lo-submission-review.md](journey-walks/2026-08-23-lo-submission-review.md)) | ✅ pass-with-findings — machinery sound (dual AUS, package hash == download, full transition table, audit chain); **WF2-F4 CLOSED — was already fixed when it was recorded** (re-verified 2026-08-12): the URLA section-4 write path landed in `6407119` (#400) on 2026-08-05, the same day this row was written. Chain on main: `urla/PropertySection.tsx:180,204` Selects → `URLAForm.tsx` `loanDetails` state → `buildPayload` → `server/routes/borrower/urla.ts:587` `urlaLoanDetailsSchema.safeParse` (400 on invalid — never a guessed value) → `updateLoanApplication` → `recalculateDecision`. Dated with `git log -S "loanDetailsChanged" -- server/routes/borrower/urla.ts`, which points only at `6407119`. **Date a standing claim before re-reporting it** ([routines/CHARTER.md](../routines/CHARTER.md) §1); **WF2-F5 CLOSED 2026-08-23** — `SubmissionLifecycleControl.tsx` ships the status-machine UI (its header documents WF2-F5) and the 2026-08-23 run drove it end to end over its endpoint; TRID blind spot fixed #395. **2026-08-23 re-run:** every gate held again (readiness refusal pre-AUS, duplicate-active 422, demo-counterparty 422, illegal-transition 422s, `funded` requires funding figures) and D1C relief was withheld for the right reasons. **New:** the live package's XSD diagnostic returned `valid:false` on `QUALIFICATION_DETAIL` — an offender the XSD suite's fixture never emits (parked PF-3); the submission machine advanced `conditions_cleared` with a transcribed condition still outstanding (trusts the operator — observation, and the lender's machine is authoritative); the **pipeline-side extension** (beyond the 8 steps) reproduced clear_to_close AND funded granted with an outstanding `prior_to_funding` condition on the only client-reachable status route — fixed same-session by the condition gate (see [../compliance/LO_SUBMISSION_TO_CLOSING_REVIEW.md](../compliance/LO_SUBMISSION_TO_CLOSING_REVIEW.md) §2.6) |
