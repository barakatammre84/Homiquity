...
| ux-06 | UX/1 | ux-refinement | P2 | no | 9 calculator pages + `Resources.tsx` + the shared `ConversionCTA` component navigate to gated routes (`/apply`, `/refinance`, `/rates`) without checking `PRELAUNCH_GATED`; while the gate is live (true in prod as of 2026-08-05), clicking silently `replace`-redirects to the generic `Waitlist.tsx` with no acknowledgment, discarding the user's calculator inputs — `Glossary.tsx`/`ArticleDetail.tsx`/`DownPaymentWizard.tsx`/`CalculatorsHub.tsx` correctly adapt their CTA instead | `pages/calculators/{Mortgage,Affordability,Amortization,Bah,DownPayment,HomeEquity,MortgagePayoff,RentVsBuy}Calculator.tsx`, `RentToOwnReadiness.tsx`, `education/Resources.tsx`, `components/ConversionCTA.tsx` (verified — finding-verifier CONFIRMED 2026-08-05) | open |
| public-funnel-01 | 1 | coverage-gap | P2 | no | 8 ungated calculator sub-routes (`/calculators/{mortgage,rent-vs-buy,affordability,amortization,home-equity,payoff,down-payment,bah}`) are absent from `SITEMAP_STATIC_PATHS`/`STATIC_ROUTE_META` — not covered by the file's documented gated-route exclusion since these routes aren't gated; invisible to `sitemap.xml` and get generic bot-rendered meta | `shared/seo/routeMeta.ts:192-203,35-109` (verified — finding-verifier CONFIRMED 2026-08-05) | open |
| public-funnel-02 | 1 | defect | P2 | no | `MortgageCalculator.tsx` and `RentVsBuyCalculator.tsx` — likely the two highest-search-volume calculators — never render `<SEOHead>`, unlike the other 6 sibling calculators; both bot- and client-rendered `<head>` fall back to generic homepage title/description | `pages/calculators/MortgageCalculator.tsx`, `pages/calculators/RentVsBuyCalculator.tsx` (verified — finding-verifier CONFIRMED 2026-08-05) | open |
