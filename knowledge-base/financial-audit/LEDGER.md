...
| `F-0814-01` | unit economics / risk | **High** | The borrower-paid compensation rule reaches **1 of its 3 call sites**. `evaluateCompensationVariance` documents that omitting `compensationModel` "reads every borrower-paid file as a lender short-pay (F-0811-05)" — and the staff **discrepancy worklist** (`submissions.ts:378`) and the **funding status machine** (`lenderSubmission.ts:357`) both omit it. Only the internal roll-up passes it. Executed: same $400k file returns `as_expected` on the roll-up path and `short_paid variance=-8000` on the worklist path, in the same HTTP response. Staff are dispatched to collect from a lender that owes nothing, at the file's full comp amount, on 100% of the borrower-paid cohort | `open` | [2026-08-14 log](../logs/2026-08-14-financial-architecture-reaudit-revenue-fix-propagation.md) §F-0814-01; `shared/compensationLedger.ts:67-76,284`; `server/routes/underwriting/submissions.ts:378,383`; `server/services/lenderSubmission.ts:357` |
| `F-0814-02` | unit economics | **High** | The 08-12 two-channel revenue fix reached **one of the two revenue surfaces**. `getAdminStats` never selects `compensationModel` (`stats.ts:90-101`, six columns, not among them) and never calls `recognizeRevenue`, so the admin dashboard carries both the phantom short-pay at portfolio scale *and* the pre-fix single-channel picture — the ~$2,000/file platform-fee channel that three audits raised is absent from the company's headline financial panel. Executed: admin shape `variance=-8000 shortPaid=1` vs. correct `variance=0 shortPaid=0` | `open` | [2026-08-14 log](../logs/2026-08-14-financial-architecture-reaudit-revenue-fix-propagation.md) §F-0814-02; `server/storage/stats.ts:89-134` vs. `server/routes/underwriting/submissions.ts:410-446` |
| `F-0814-03` | balance sheet | Medium | **Lender over-payment is recognized as revenue in full and carried as no liability.** Confirms and extends `F-0810-05` (was rank 4, unverified at HEAD). `evaluateCompensationVariance` warns "lenders claw back overpayments"; `summarizeCompensation:279` and `recognizeRevenue:170` both recognize the excess anyway — and `revenueRecognition.ts` was **written 08-12, after the finding was raised**, inheriting the behaviour. There is no `overPaidCount` mirroring `shortPaidCount`, and `over_paid` appears in `contingentLiabilities.ts` nowhere. Standing signal #2: the measurement fails in the same direction as the defect | `open` | [2026-08-14 log](../logs/2026-08-14-financial-architecture-reaudit-revenue-fix-propagation.md) §F-0814-03; `shared/compensationLedger.ts:144-147,279`; `shared/revenueRecognition.ts:170` |
| `F-0814-04` | capital flow / liquidity | Medium | **The remittance lag is a fabricated zero.** `cycleTimeReport.ts:210` deliberately reports days-to-cash as *unknown* rather than equal to the funding cycle — "the lender's wire lag is not zero, it is unmeasured". Its only writer, `lenderSubmission.ts:365`, stamps `compensationReceivedAt: fundedAt` — the same instant — and the `receivedAt >= fundedAt` filter accepts it as a measured same-day wire. Executed: as-written gives `remittanceLag=0, daysToCash=30, guard note NO`; NULL gives `null/null/YES`. For a broker with no warehouse line the remittance float **is** the working-capital requirement — order of **$75k–$110k reported as $0** at 20 files/mo × $8k × 14 days | `open` | [2026-08-14 log](../logs/2026-08-14-financial-architecture-reaudit-revenue-fix-propagation.md) §F-0814-04; `server/services/lenderSubmission.ts:356-366`; `shared/cycleTimeReport.ts:166-190,210` |
...
| 4 | `F-0810-05` → **`F-0814-03`** | Over-paid compensation is booked as revenue and carried as no liability | **Re-verified by execution 2026-08-14 and superseded by `F-0814-03`**, which adds the two things the original did not have: `recognizeRevenue` (written *after* this finding) inherited the behaviour, and there is no `overPaidCount` at all. Track it as `F-0814-03` |
...
| 2026-08-14 t5 | `1f520b1` | audit | Position 0/0, no open PRs from this routine. Diff since the last audit was a mechanical `apiRequest` convergence — **zero money-bearing files changed**, so the diff-driven pass yielded nothing and the tick ran on standing signal #1 instead. Four new findings, all one root: **the 08-12 remediation landed on one consumer of each rule and the siblings kept the old behaviour** — `evaluateCompensationVariance` fixed at 1 of 3 call sites (`F-0814-01`), `recognizeRevenue` wired into 1 of 2 revenue surfaces (`F-0814-02`), over-payment classified in one module and recognized in two (`F-0814-03`), the remittance trigger guarded in the reader and fabricated by the writer (`F-0814-04`). All four confirmed by executing the repo's own functions. 79 money-path tests green **through every one of them** — the suite exercises the correct call shape, so it cannot see a caller that omits the argument. F-16 asset-light re-verified sound by direct grep. No fix: no ledger row was at `authorized` (R7) |
