...
- [logs/2026-08-08-financial-architecture-reaudit-commission-payouts.md](logs/2026-08-08-financial-architecture-reaudit-commission-payouts.md) — third financial re-audit, the first to walk the money going **out**. The prior passes audited lender compensation, the fee schedule and the QM cap; none read `broker_commissions`, a live payout table whose rows reached no margin figure, no balance sheet and no audit log while `costLedger.ts` asserted commissions were "not captured anywhere" (F-20, fixed: payouts now charged against gross margin by lifecycle state, approved-but-unpaid booked as a register payable, and disbursed commission inside a live EPO window reported as loss *on top of* the clawback rather than netted). Open: the payout is struck as **0–10% of loan amount with no reference to the file's compensation and no funded gate** — permitting a $25,000 payable against $5,000 of revenue — and its Reg Z §1026.36(d)(1) / RESPA §8 posture needs counsel (F-21, ledger `regz-1026-36d1-referral-commission-payout`, 14-day interval); borrower-paid platform fee income (~$2,000/file) is absent from the revenue line, so **both sides of the margin were wrong in opposite directions** (F-22); and the cash-conversion cycle has every operand recorded — including the unread `compensation_received_at` — and is computed nowhere (F-23). Money-out routes are now audited (F-24); four-eyes on disbursement left as a founder call.
