...
bare pair survives as `pnpm test:raw`, `:16`). The **node** lane, whose `include` is a
...
`.githooks/pre-push` (typecheck + eight guards; the unit lanes only under `PREPUSH_TESTS=1`; an
...
flowchart TD
  ...
  C --> D["15 scripts/*-guard.cjs against 7 baselines - text scans; counts may only go down"]
  ...
  P[".githooks/pre-push - tsc + 8 guards - unit lanes only under PREPUSH_TESTS=1"] -. covers .-> A
...
- **The gate job.** `.github/workflows/ci.yml:118` `name: gate (typecheck · tests · schema guard)`
  (`od -tx1` shows `c2 b7` = U+00B7; rename procedure at `:108-117`); triggers `:54-101` —
...
- **The 18 gate steps, in order** (`grep -n '^      - name:' .github/workflows/ci.yml`): change
  ...
  two dots was F-0818-16, `:73-78`) → `guard:querykeys` (`:423`) → guard scripts parse (`:447` —
  added after a backtick inside `browser-probe.cjs` made every probe crash and a sweep read the
  stack trace as CLEAN, `:450-456`) → production build (`:465`) → `guard:bundle` (`:484`) →
  self-host boot (`:510`, `PORT=3999`, poll `/api/health` 45 × 1 s). Six doc-side steps run even on
  a docs-only PR (`:211-215`); the change-scope step fails closed (`:217-220`).
- **Pre-push.** `.githooks/pre-push` — `grep -c '^step ' .githooks/pre-push` → `9`: typecheck
  ...
  (`:130-134`); the default branch prints `skipped — CI runs them`. The stated reason is capacity,
...
- **Preflight.** `scripts/preflight.sh` — `grep -c 'step "' scripts/preflight.sh` → `18` (its
  header says "sixteen"); stages `:80-189`: pre-push armed → tsc → the eight guards → guard-scripts
  parse → query-key → §9 security review (only when `origin/main` is fetched, else SKIPPED) → unit
  tests + collection floor (`:121`) → `pnpm audit` → build → bundle ratchet → self-host boot
  (`BOOT_PORT 3999`) → integration lane (`INT_PORT 4000`, **dev mode on the production bundle**
  because `/api/test-login` 404s in production, `:178-182`). `--fast` skips
  build/bundle/boot/integration; `--no-db` skips the two DB stages; **SKIPPED is a third state that
  neither passes nor fails** (`:22`, `:64-66`, `:213-216`); every run prints what it cannot see
  (`:198-206`).
...
- **Gaps between the lanes.** Neither pre-push nor preflight runs `guard:citations`
  ...
  three query-key scripts (`pre-push:117`, `preflight.sh:94` call `query-key-guard.cjs` directly;
...
- **Ports.** Dev 5001 (from `.env.example`; code default 5000; AirPlay squats on 5000), worktree
  ...
  (`knowledge-base/runbooks/LOCAL_DEV.md:183-187`, `scripts/preflight.sh:38-39`, `ci.yml:526`).
...
# → 63 / 16 @ 6377727e
sed -n '118p' .github/workflows/ci.yml | od -An -tx1 | sed -n '2p'
# → 74 79 70 65 63 68 65 63 6b 20 c2 b7 20 74 65 73 — `c2 b7` is the U+00B7 in the check name @ 6377727e
