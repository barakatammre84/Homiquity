...
3. T5: `curl -s https://homiquity-production.up.railway.app/api/health | jq -r .commit` must equal
   the merge SHA. Check the `if:` lines of the `migrate-prod` and `verify-deploy` jobs in
   `.github/workflows/ci.yml` (`grep -n "if:" .github/workflows/ci.yml`): while `verify-deploy`
   is switched off and `migrate-prod` runs only on workflow dispatch, a PR that carried a
   migration needs the CI workflow run by hand with `dry_run=false`, and
   `applied N migration(s)` read in its log. Chapter 07 keeps the dated status of both jobs.
