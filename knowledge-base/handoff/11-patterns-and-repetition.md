...
| B11 | **Tighten, never loosen**: a lesson or a correction may move toward a compliance rail, never away. | `LESSONS.md:17-20`; `doc-accuracy/SKILL.md:57` D8 | R5, R8, R12. |
| B12 | **Drift vs regression** (doc-accuracy D7): a doc stating an invariant the code violates may be reporting a regression — do not edit the doc to match. | `doc-accuracy/SKILL.md:50-55` | `prompts/doc-update.md` step 1. |
...
| B16 | **No self-amendment**: a routine may never edit its own skill file. | `doc-accuracy/SKILL.md:71` D10 | `_RAILS.md` R12: never edit `.claude/**`. |
