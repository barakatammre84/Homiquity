...
flowchart TD
  ...
  J --> M["Plaid and verify - plaid.ts, statusDecisions.ts:330 verify-financials - three provenance booleans"]
  ...
  P --> R["denied: ensureAdverseActionForDenial BEFORE the flip - statusDecisions.ts:215 - 422 if it cannot"]
...
| 6 | Intake automation | runs after `res.status(201)` (`:279` → `:293`) | `server/services/loanAnalysis.ts:426` `finalizeIntake` — writes `status: "analyzing"` (`:436`) then the outcome (`:453`) **directly**; resets to `submitted` on failure (`:588`) for the recovery sweep (`:596-614`); automation never denies — the non-approval outcome is `under_review` (`:400`) | `loan_applications.status`, `preApprovalAmount`, `dtiRatio`, `ltvRatio`, `aiAnalysis` |
...
| 9 | Credit pull | `POST /api/loan-applications/:id/credit/pull` `compliance.ts:603` — active consent required (`:622` → 400), `pullType` is a required `z.enum(["soft","hard","tri_merge"])` (`:632-635`; it used to default to the most invasive option), scope mismatch → 403 `CONSENT_SCOPE_MISMATCH` (`:662`) | `server/services/creditPulls.ts` — `isSimulated: creditVendorIsSimulated()` stamped **at INSERT** (`:107`, derived from `!process.env.CREDIT_VENDOR_API_KEY` `:27-29`); marks the credit dimension verified (`compliance.ts:690`) | `credit_pulls`, `credit_audit_log` + `credit_audit_chain_tips`, `loan_cost_entries` |
| 10 | Verification | `server/routes/lending/statusDecisions.ts:330` `/verify-financials` (all dimensions) and `:381` `/verify/:dimension`; Plaid routes `compliance.ts:23,75` | `server/plaid.ts` (link token, exchange, identity, remove, get — 178 lines); `server/services/verification.ts:23` `markDimensionVerified` promotes `financialDataProvenance` to `verified` when all three are true (`:37-40`) | `verification_reports`, `verifications`, `plaid_link_tokens`, the three `*Verified` booleans |
...
| 13 | Pipeline | `PATCH /api/loan-applications/:id/status` `statusDecisions.ts:127` (staff; `STAFF_SETTABLE_STATUSES`); `POST …/advance-stage` `server/routes/underwriting/pipeline.ts:265` | `server/pipelineEngine.ts:594` `updatePipelineStage` — no-op when unchanged (`:615`), `PipelineTransitionError` (`:574`) → 409 with `allowedStatuses` (`statusDecisions.ts:245-252`), `force` is admin-only and skips **only** the table (`:600`, `statusDecisions.ts:242`) | `loan_applications.status`; milestones (`:623-655`), HMDA action codes 1/3/4 (`:670-678`), outcome stamp (`:687-692`), state-machine sync (`:701-706`), task event (`:709-736`), homeowner graduation on `funded` (`:648-654`) |
| 14 | Adverse action | inside hop 13: `statusDecisions.ts:212-231` — `ensureAdverseActionForDenial` runs **before** the flip, 422 if it cannot; HMDA needs ≥ 2 reasons (`:145-148`); only underwriter/admin set outcomes (`:155`) | `server/services/creditAdverseActions.ts:521`; delivery sweep `adverseActionDelivery.ts` via the cron | `adverse_actions`, `credit_audit_log`, `hmdaActionTaken` / `hmdaDenialReasons` |
| 15 | TRID | the TRID hard stop blocks forward movement, never exit dispositions (`statusDecisions.ts:202-208`) | `server/services/trid.ts` (`assessSixPieces` `:58`, `evaluateTridTrigger` `:160`), `loanEstimate.ts:486` `generateLoanEstimate`, `changeOfCircumstance.ts:63`, `shared/compliance/feeTolerance.ts` | `loan_estimate_disclosures`, `change_of_circumstances`, `loan_cost_entries`; `tridTriggeredAt` written only by `trid.ts` |
...
# → 1   (the app-guide chapter names none of the current chokepoints) @ 074899e3
```
...
| **`finalizeIntake` compensates by hand for the side effects it bypasses** — its own state-machine sync (`loanAnalysis.ts:437-451`) and outcome stamp (`:465-471`); milestones, HMDA codes and task events are *not* emitted for the intake hop. Add a side effect to the pipeline engine and `finalizeIntake` diverges silently. | `server/services/loanAnalysis.ts:436,453,588` vs `server/pipelineEngine.ts:623-736` | `tests/statusVocabulary.test.ts:243` permits the bypass by name; nothing checks the two stay in sync. |
