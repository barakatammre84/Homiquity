...
| 12 | Decision | triggered from 8 files — `grep -rn "recalculateDecision(" server --include='*.ts' | wc -l` → `11` | `server/services/decisionEngine.ts:449` `recalculateDecision` — best-effort, never throws into the caller (`:498-503`) | `decision_snapshots` (`:479`), `income_path_evaluations` (`:478`) — append only |
