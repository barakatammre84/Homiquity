...
- **The routine skeleton** (read `.claude/skills/refactor-radar/SKILL.md`, 277 lines, or
  `doc-accuracy/SKILL.md`, 343): frontmatter with only `name` + the anti-autoload `description` →
...
- **The charter.** `knowledge-base/routines/CHARTER.md` — 855 lines, 14 H2s. §1b (`:120-147`):
  ...
  (`financial-audit/SKILL.md:24`, `doc-accuracy/SKILL.md:49-53`); CHARTER §5's clock is 24 h
...
- **Four Markdown linters.** `package.json:37,40,41,42`: `guard:docs` (`scripts/doc-freshness-guard.cjs`
  ...
  first 53 hits were correct as written, `:21-38`). None reads meaning — `doc-accuracy/SKILL.md:23-24`:
...
| The guards cannot see meaning: a doc can be green on all four while telling readers to run commands that no longer exist. The mitigation (doc-accuracy) is itself prose. | `doc-accuracy/SKILL.md:23-24` | By design. |
