...
| "CI runs on every push" | **True — but enforcement is procedural** *(re-corrected 2026-07-19 evening)*: `.github/workflows/ci.yml` runs the **`gate`** check (typecheck · unit tests · blocking prod audit · schema guard · design-token ratchet) on every PR, and the `migrate-prod` job auto-applies migrations on merge ([CICD.md](../runbooks/CICD.md)). GitHub does **not** enforce the gate: the repo is deliberately private on the Free plan (no branch protection), so merges wait for green by the watch-then-merge recipe, not platform rules ([TEAM_PRACTICES](./TEAM_PRACTICES.md) §6). Integration tests stay manual — CI never runs them. | — |
