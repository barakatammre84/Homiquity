...
| `arbitraryTypeScale` | 0 | **153.** The real backlog. A bespoke `text-[11px]`/`w-[240px]` is a rung outside the scale §3 owns — and below `text-xs` it is also a mobile legibility problem, which is where the cheap wins are. |
| `subMinTouchTarget` | 0 | **233.** Two limits, stated so nobody reads a zero as "touch targets are fine": `.touch-target` is scoped `max-width: 767px`, so a 768px+ touch device is **not** covered; and raw `<button>`s are excluded as too noisy to fail on (33 of them are reported in §0 for human review). |
