...
| RR-015 | client/src/pages/calculators/AmortizationCalculator.tsx | 519 | ~140-line pure block (`simulate` payoff loop + `calculate` schedule builder + 4 interfaces + `defaultInputs`) inline in the page, while both sibling calculators already extracted (`lib/affordabilityEstimate.ts`, `lib/rentVsBuyEstimate.ts` via RR-012). No query keys in the file at all | move the pure block verbatim to `client/src/lib/amortizationEstimate.ts` with colocated characterization tests, mirroring `rentVsBuyEstimate.ts`; page keeps all JSX/state/mutation | low — pure functions, zero query keys, no JSX/route/copy change; the one `@shared/lib/amortization` call moves untouched | in-pr | https://github.com/barakatammre84/Homiquity/pull/528 — page 519→378 lines | 2026-08-17 |
| RR-016 | client/src/pages/admin/Lenders.tsx | 685 | not yet audited (new — 4th-largest client file, absent from the 2026-08-08 seed sweep) | tbd | tbd | open | — | 2026-08-17 |
...
| 2026-08-17 | full | RR-015 client/src/pages/calculators/AmortizationCalculator.tsx | code PR opened | https://github.com/barakatammre84/Homiquity/pull/528 | 1/5 | First scheduled (Sun 20:00) run. Research cache 5 days old → reused per the 30-day policy, no fetching. Golden values for the characterization tests were generated by executing `origin/main`'s ORIGINAL inline implementation in a scratch harness, so a drifted "verbatim" copy would have failed the test rather than passing vacuously; the moved block was also `diff`-proven byte-identical. Re-measured the size sweep: URLAForm 625→750, PreApproval 882→904, and `admin/Lenders.tsx` (685) surfaced as a new 4th-largest file the 2026-08-08 seed missed → RR-016. |
