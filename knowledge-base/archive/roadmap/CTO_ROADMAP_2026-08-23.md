> **⛔ ARCHIVED 2026-08-23 — a verbatim snapshot of `CTO_ROADMAP.md` as it stood at `c8bb44f`.
> Do not act on this document.** The live file was rewritten the same day on the founder's
> development-first directive (CHARTER §1a, 2026-08-23): the Fannie Mae *Selling Guide* became
> the base of the roadmap, and the launch-shaped partition this file encodes — billing above
> engineering, "blocks go-live", "launch-blocking", "post-flip", "blocked on a contract — do NOT
> start" — was withdrawn as a ranking frame. Business events now live in one founder lane that
> never ranks, pauses, or gates engineering; every open item below was migrated into the
> successor, none closed by this rewrite.
>
> **Corrections recorded here, never applied to the snapshot text (TEAM_PRACTICES §2):**
> 1. **F14's Selling Guide leg was satisfied before this snapshot** — B3-2-02 and B3-2-03 are in
>    the committed section index as of 2026-08-20 (the corpus landed in #650/#708); the trigger
>    text below did not know it.
> 2. **1.12 is partly executed** — the Reg Z capture landed 2026-08-20 (`docs/reg-z/` holds 12
>    CFR 1026 + Supplement I; three ledger entries re-verified). The residue moved to the
>    successor's 1.12.
> 3. **KTLO-1, KTLO-2 and KTLO-4 were superseded by evening-triage re-measurements riding
>    PR #698** (open at snapshot time; its roadmap delta — the KTLO rewrites, 3.24's push-before-
>    exit fix, 3.31's re-measurement, new items 3.32–3.39, and 3.29's completion-deletion — is
>    incorporated into the successor rather than lost): Railway never stopped serving and prod
>    tracks `origin/main`; `migrate-prod`/`verify-deploy` were re-armed by #669 after the 0057
>    outage; Actions is alive because the repo is public, with the payment failure unresolved
>    beneath it.
